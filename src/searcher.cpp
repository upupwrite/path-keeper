...
static bool parseSelection(const std::string& line, int& dirDisp, int& cmdDisp) {
    ...
    std::string indexPart = (tabPos != std::string::npos) ? line.substr(0, tabPos) : line;
...
std::string Searcher::interactiveSearch(const Json::Value& config, File& file) {
    ...
    if (useFzf) {
        ...
        std::string cmd = std::string("fzf --delimiter='\t' --with-nth=2,3 < ") + tmpname;
        ...
        std::cerr << list;
        std::cerr << Colors::CYAN << "请输入要执行的编号 (例如 5.1): " << Colors::RESET;
        std::getline(std::cin, selectedLine);
    }

    if (selectedLine.empty()) return "";

    int dirDisp = 0, cmdDisp = 0;
    if (!parseSelection(selectedLine, dirDisp, cmdDisp)) {
        std::cerr << Colors::RED << "无效的选择格式" << Colors::RESET << std::endl;
        return "";
    }
    ...
    if (dirDisp < 1 || dirDisp > static_cast<int>(validDirs.size())) {
        std::cerr << Colors::RED << "目录编号超出范围" << Colors::RESET << std::endl;
        return "";
    }
    ...
    if (cmdDisp < 1 || cmdDisp > static_cast<int>(cmds.size())) {
        std::cerr << Colors::RED << "命令编号超出范围" << Colors::RESET << std::endl;
        return "";
    }
