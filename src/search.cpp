...
static bool parseSelection(const std::string &line, int &dirDisp,
                           int &cmdDisp) {
  // 期望格式：行首是 "5.1" 后跟 tab
...
SearchResult Searcher::interactiveSearch(const Json::Value &config,
                                         File &file) {
  // 确保 key order 已加载
  file.load_key_order();
  ...
  if (useFzf) {
    ...
    if (fd == -1) {
      std::cerr << Colors::RED
                << QCoreApplication::translate("Searcher", "无法创建临时文件")
                       .toStdString()
