...
void Interaction::main(int argc, char **argv) {
  try {
    ...
    if (option == "-a" || option == "--add") {
      ...
      if (argc > 2) {
        ...
        if (std::strcmp(argv[2], "--enable") == 0) {
          if (argc >= 4 && std::strcmp(argv[3], "global") == 0) {
            ...
            if (!pk.parseIndex(target, valid_dirs, config["path"], directory,
                               cmd_idx)) {
        ...
        if (std::strcmp(argv[2], "-editor") == 0) {
          ...
          SETEDITOR:
            ...
            if (!currentEditor.empty()) {
              std::cerr << "Current editor setting: " << currentEditor << "\n";
            }
            auto editors = editor.getAvailableEditors();

            if (editors.empty()) {
              std::cerr << "No common editors found on your system.\n";
            ...
            std::string selectedEditor = editor.getUserChoice(editors);
      ...
      std::cerr << Colors::RED
                << QCoreApplication::translate("Interaction", "未知选项: ")
                       .toStdString()
                << option << Colors::RESET << std::endl;
      std::cerr
          << QCoreApplication::translate("Interaction", "用法").toStdString()
          << ": pk [-a | -s | -p "
             "| -c | -e "
             "[index]]"
          << std::endl
