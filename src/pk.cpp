...
void PathKeeper::outputCommand(const std::string& cmd)
{
    if (!cmd.empty())
        ...
        std::cerr << Colors::YELLOW
                  << QCoreApplication::translate("PathKeeper", "无法生成命令")
                         .toStdString()
...
void PathKeeper::processIndexSelection(const std::string& index_str,
                                       ...
                                       bool output_command, bool set_recent)
{
    ...
    if (output_command)
    {
        std::string final_cmd = CommandBuilder::build(
            directory, command, logger, log_set, log_value);
...
void PathKeeper::addRecord()
{
    ...
    if (cmd.empty())
    {
        std::string choice = ReadlineHelper::read_line(
            Colors::CYAN + std::string("按 'e' 打开编辑器，其他键输入单行: ") +
            Colors::RESET);
        if (choice == "e" || choice == "E")
        {
            ...
            if (fd == -1)
            {
                std::cerr << Colors::RED << "无法创建临时文件" << Colors::RESET
                          << std::endl;
                return;
            }
            close(fd);
            ...
            {
                std::cerr << Colors::RED << "编辑器 '" << editor_cmd
                          << "' 不存在或无权执行！" << Colors::RESET
                          << std::endl;
                unlink(tmpname);
                return;
            }
            ...
            std::cerr << Colors::GREEN << "正在启动编辑器: " << editor_cmd
                      << Colors::RESET << std::endl;
            ...
            if (ret == -1)
            {
                std::cerr << Colors::RED << "调用编辑器失败 (fork 失败)"
                          << Colors::RESET << std::endl;
                unlink(tmpname);
                return;
            }
            if (ret != 0)
            {
                std::cerr << Colors::RED << "编辑器异常退出 (返回码: " << ret
                          << ")" << Colors::RESET << std::endl;
                unlink(tmpname);
                return;
            }
            ...
            file.saveConfig(config);
            std::cerr << Colors::GREEN
                      << QCoreApplication::translate("addRecord", "记录已保存!")
                             .toStdString()
                      << Colors::RESET << std::endl;
        ...
        else
        {
            ...
            cmd = ReadlineHelper::read_line(
                Colors::CYAN +
                QCoreApplication::translate("addRecord", "请输入单行命令:")
                    .toStdString() +
                Colors::RESET);
        }
    }
    ...
    if (!cmd.empty())
    {
        std::string logChoice = ReadlineHelper::read_line(
            Colors::CYAN +
            std::string("为该命令单独设置日志记录? (y=强制记录 / n=强制不记录 "
                        "/ 回车=跟随全局): ") +
            Colors::RESET);
...
void PathKeeper::outputRecentCommand()
{
    ...
    std::string final_cmd =
        CommandBuilder::build(directory, command, logger, log_set, log_value);
    outputCommand(final_cmd);
}
...
void PathKeeper::setRecent()
{
    ...
    std::string index_str = getInputIndex(
        "", QCoreApplication::translate("setRecent", "请输入目标编号: ")
                .toStdString());
    if (index_str.empty())
        return;
...
void PathKeeper::selectRun(const std::string& cmd_index, bool set_recent,
                           bool show)
{
    Json::Value config = file.loadConfig();
    if (show)
