...
void PathKeeper::runRecent() {
  ...
  if (!config["recent"].isArray() || config["recent"].size() != 2) {
    std::cerr << Colors::RED
              << QCoreApplication::translate("runRecent", "最近记录格式无效!")
                     .toStdString()
              << Colors::RESET << std::endl;
    return;
  }
  ...
  try {
    ...
    if (command.empty()) {
      std::cerr
          << Colors::RED
          << QCoreApplication::translate("runRecent", "命令无效!").toStdString()
          << Colors::RESET << std::endl;
      return;
    }
  ...
  if (show) {
    ...
    for (const auto &directory : valid_dirs) {
      std::cerr << Colors::BLUE << "[" << i << "]" << Colors::RESET << " "
                << Colors::GREEN << directory << Colors::RESET << std::endl;
...
void PathKeeper::runCommand(const std::string &directory,
  ...
  if (access(directory.c_str(), F_OK) != 0) {
    std::cerr << Colors::RED
              << QCoreApplication::translate("runCommand", "目标目录不存在!")
                     .toStdString()
              << Colors::RESET << std::endl;
    return;
  }
  ...
  RUN:
      ...
      if (file.isCommandLogEnabled(directory, idx)) {
        std::cerr << Colors::YELLOW
                  << QCoreApplication::translate("runCommand", "执行命令")
                         .toStdString()
                  << QCoreApplication::translate("runCommand", "(录制中): ")
                         .toStdString()
    ...
    std::cerr << Colors::RED
              << QCoreApplication::translate(
                     "runCommand", "这个命令似乎被更改过,是否信任执行(Y/n): ")
                     .toStdString()
              << Colors::RESET;
...
void PathKeeper::displayCommands(const Json::Value &commands,
                                 int parent_index) {
...
bool PathKeeper::parseIndex(const std::string &index_str,
  ...
  if (index_str.find('.') != std::string::npos) {
    ...
    try {
      ...
      if (display_num < 1 || display_num > static_cast<int>(valid_dirs.size()))
        return false;
      ...
      return (cmd_idx >= 0 && cmd_idx < static_cast<int>(commands.size()));
    } catch (const std::exception &) {
      return false;
    }
  } else {
    try {
      ...
      if (commands.size() > 1) {
        std::cerr
            << Colors::GREEN
            << QCoreApplication::translate("parseIndex", "目录:").toStdString()
            << Colors::RESET << " " << directory << std::endl;
        displayCommands(commands);
...
void PathKeeper::search() {
  ...
  if (!result.valid) {
    std::cerr << Colors::YELLOW
              << QCoreApplication::translate("runSearch", "未选择任何命令。")
                     .toStdString()
              << Colors::RESET << std::endl;
    return;
  }
